...
public class SG90 extends ServoMotor implements Motor {
	...
	public void setSpec(){
		...
		minTheta = -Math.PI*0.55;				// モーターの最小回転角度. rad
		maxTheta = Math.PI*0.55;					// モーターの最大回転角度. rad
		minThetaLimit = -Math.PI*0.2;				// モーターの最小制限回転角度. rad
		maxThetaLimit = Math.PI*0.2;				// モーターの最大制限回転角度. rad
